...
<div class="p-4 bg-white block sm:flex items-center justify-between border-b border-gray-200 lg:mt-1.5 dark:bg-gray-800 dark:border-gray-700 transition-all duration-300">
    <div class="w-full mb-1">
        ...
        <div class="items-center justify-between block sm:flex gap-4">
            ...
            <button id="createCategoryButton" class="text-white bg-primary-700 hover:!bg-primary-800 focus:ring-4 focus:!ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:!bg-primary-600 dark:hover:!bg-primary-700 focus:outline-none dark:focus:!ring-primary-800 transition-all duration-300 flex items-center justify-center gap-2 w-full sm:w-auto" type="button" data-drawer-target="drawer-create-category-default" data-drawer-show="drawer-create-category-default" aria-controls="drawer-create-category-default" data-drawer-placement="right">
...
@if(session('success'))
<div class="p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400 animate-fade-in-down" role="alert">
    {{ session('success') }}
</div>
@endif
                    ...
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th scope="col" class="p-4 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'name', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}" class="flex items-center hover:text-primary-600 dark:hover:text-primary-400 transition-colors duration-300">
                                    Category Name
                            ...
                            <th scope="col" class="p-4 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'created_at', 'direction' => request('direction') === 'asc' ? 'desc' : 'asc']) }}" class="flex items-center hover:text-primary-600 dark:hover:text-primary-400 transition-colors duration-300">
                                    Created At
                            ...
                            <th scope="col" class="hidden md:table-cell p-4 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">
                                Description
                            </th>
                            <th scope="col" class="p-4 text-xs font-medium tracking-wider text-left text-gray-500 uppercase dark:text-gray-400">
                                Actions
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-300">
                            ...
                            <td class="p-4 space-x-2 whitespace-nowrap">
                                <div class="flex flex-col sm:flex-row gap-2">
                                    <button type="button" data-drawer-target="drawer-update-category-default" data-drawer-show="drawer-update-category-default" aria-controls="drawer-update-category-default" data-drawer-placement="right" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:!bg-primary-800 focus:ring-4 focus:!ring-primary-300 dark:!bg-primary-600 dark:hover:!bg-primary-700 dark:focus:!ring-primary-800 transition-all duration-300" onclick="editCategory({{ $category->id }})">
                                        <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z"></path><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd"></path></svg>
                                        Update
                                    </button>
                                    <button type="button" onclick="deleteCategory({{ $category->id }})" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:!bg-red-800 focus:ring-4 focus:!ring-red-300 dark:!focus:!ring-red-900 transition-all duration-300">
                        ...
                        <tr>
                            <td colspan="4" class="p-4 text-center text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center justify-center py-8">
                                    ...
                                    <p class="text-sm text-gray-500 mt-2">Start by adding a new category</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
...
<!-- Add Category Drawer -->
<div id="drawer-create-category-default" class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs sm:max-w-md p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
    ...
    <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col h-full">
        @csrf
        <div class="space-y-4">
            <div>
...
<div id="drawer-update-category-default" class="fixed top-0 right-0 z-40 w-full h-screen max-w-xs sm:max-w-md p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
    ...
    <form id="updateCategoryForm" method="POST" enctype="multipart/form-data" class="transition-all duration-300">
        @csrf
        @method('PUT')
        <div class="space-y-4">
            <div>
            ...
            <div>
                ...
                <div id="current_image" class="mt-2">
                    ...
                    <img id="current_image_preview" src="" alt="Current category image" class="mt-1 max-h-32 rounded-lg">
                </div>
            </div>
...
function deleteCategory(id) {
    Swal.fire({
        ...
        showClass: {
            popup: 'animate-fade-in-down'
        },
        hideClass: {
            popup: 'animate-fade-out-up'
        }
    }).then((result) => {
...
#drawer-create-category-default, #drawer-update-category-default {
